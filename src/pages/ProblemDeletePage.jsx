...
const TagsInput = ({ control, name, errors }) => {
    const allTags = [
        ...
        "Mathematics", "Basic Operations", "BFS", "DFS", "problemData Structures", "Strings", "Binary Search"
    ];
...
export default function ProblemCreate() {
  ...
  try {
    ...
    reset({
      ...
      HiddenTestCases: data.HiddenTestCases?.length
        ...
        : [{ input: '', output: '' }],
      startCode: ['C++', 'Java', 'JavaScript'].map(lang => ({
        language: lang,
        initialCode:
          data.startCode?.find(item => item.language === lang)?.initialCode || '',
      })),
      referenceCode: ['C++', 'Java', 'JavaScript'].map(lang => ({
        language: lang,
        completeCode:
          data.referenceCode?.find(item => item.language === lang)?.completeCode || '',
      })),
    });
  } catch (error) {
                ...
                <SectionCard title="Test Cases" icon={<FiGitCommit />}>
                    <div className="p-4 rounded-lg bg-gray-50/70">
                      ...
                      {visibleFields.map((field, index) => (
                        <div key={field.id} className="bg-white/60 p-4 rounded-md mb-3 border-l-4 border-[#034078] shadow-sm">
                    ...
                    <div className="p-4 rounded-lg bg-gray-50/70 mt-4">
                       <h3 className="font-semibold mb-3 text-lg flex items-center gap-2 text-gray-700"><FiEyeOff /> Hidden Test Cases</h3>
                        {hiddenFields.map((field, index) => (
                          <div key={field.id} className="bg-white/60 p-4 rounded-md mb-3 border-l-4 border-[#1282a2] shadow-sm">
                              <div className="flex justify-between items-start">
                                  <p className="font-bold text-gray-500 mb-2">Case #{index + 1}</p>
                ...
                <SectionCard title="Code Stubs & Solutions" icon={<FiCode />}>
                  {['C++', 'Java', 'JavaScript'].map((lang, i) => (
                    <div key={lang} className="bg-gray-50/70 p-4 rounded-xl">
                      <h3 className="font-bold text-lg text-[#034078] mb-3">{lang}</h3>
                      <FormField
          as="textarea"
          rows={8}
  ...
  <h1 className="text-3xl font-bold mb-6 text-gray-800">Delete Problem</h1>
  <div className="w-full max-w-md flex flex-col sm:flex-row gap-4">
    <input
      type="text"
      className="input input-lg w-full sm:flex-1"
